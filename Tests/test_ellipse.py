...
class EllipseTest(unittest.TestCase):
    ...
    def test_init_zero_major_axis(self):
        ...
        with self.assertRaises(ValueError) as error:
            Ellipse(0.0, 3.0)
        self.assertEqual(
            str(error.exception), "Semi-major axis length must be positive."
        )
    ...
    def test_init_zero_minor_axis(self):
        ...
        with self.assertRaises(ValueError) as error:
            Ellipse(5.0, 0.0)
        self.assertEqual(
            str(error.exception), "Semi-minor axis length must be positive."
        )
    ...
    def test_init_non_numeric_major_axis(self):
        ...
        with self.assertRaises(TypeError) as error:
            Ellipse("hello", 3.0)
        self.assertEqual(
            str(error.exception), "Semi-major axis length must be a number."
        )
    ...
    def test_init_non_numeric_minor_axis(self):
        ...
        with self.assertRaises(TypeError) as error:
            Ellipse(5.0, "world")
        self.assertEqual(
            str(error.exception), "Semi-minor axis length must be a number."
        )
    ...
    def test_get_description(self):
        ...
        expected_circumference = (
            "Ellipse with axis {} and {} cm has circumference {:.3f} cm.".format(
                ...
                round(ellipse.circumference(), 3),
            )
        )
        expected_area = "Ellipse with axis {} a {} cm and has area {:.3f} cm2.".format(
            ellipse.semi_major_axis, ellipse.semi_minor_axis, round(ellipse.area(), 3)
        )
        self.assertEqual(
            ellipse.get_description(), (expected_circumference, expected_area)
        )
